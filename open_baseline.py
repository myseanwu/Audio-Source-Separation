...
### Model
from torch.nn import LSTM, Linear, BatchNorm1d, Parameter, RNN
import torch
...
class OpenUnmix(nn.Module):
    def __init__(
        ...
        self.fc2 = Linear(
            in_features=hidden_size*2, ########################### baseline *2 , Mel model *3
    ...
    def forward(self, x):
        ...
        x = torch.cat([x1, lstm_out[0]], -1) ######### baseline [x, lstm_out[0]]
...
def main():
    ...
    target_path = Path('./open-unmix/default_train_20/')
    ...
    model_path = False#'./open-unmix/default_train_20/' ###################resume
    if model_path:
        scaler_mean = None
        scaler_std = None
    else:
    ...
    #model_path = './open-unmix/default_train_20/' ###################resume
